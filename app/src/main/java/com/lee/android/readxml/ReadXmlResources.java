...
public class ReadXmlResources {
    ...
    public ReadXmlResources(Context context){
        mContext = context;
        try {
            loadPolicyFile();
...
//        InputStream infile = this.getResources().openRawResource(R.xml.notification_policy_default);
        XmlResourceParser parser = mContext.getResources().getXml(R.xml.notification_policy_default);
//        判断文件是否存在
//        if(systemDir.exists()){
//            InputStream infile = new FileInputStream(systemDir); //读取存储卡目录下面的文件
        try {
//                infile = mPolicyFile.openRead();
            readPolicyXml(parser);
...
//            Toast.makeText(this,"zhixing",Toast.LENGTH_SHORT).show();
        ...
        Toast.makeText(mContext.getApplicationContext(),"存在RXResources",Toast.LENGTH_SHORT).show();
...
//        }
    ...
    private void readPolicyXml(XmlPullParser parser)
